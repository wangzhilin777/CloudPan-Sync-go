...
- 当前最主要的缺口集中在 provider 真实联网实现，而不是内核缺失。
...
| Planner | 预览计划、策略判定、阈值判断、冲突降级、deepest-first | 已完成 | 无 | 结合真实 provider 再校准策略 |
| Task Runtime | 创建、查询、运行、暂停、恢复、重试、结果落库 | 已完成 | 无 | 真实上传链路接入后补更细运行态 |
...
- 已开始进入“逐家 provider 真实落地”阶段

## 当前最值得继续做的三件事

1. 把 `aliyundrive_open` 的目录链路补成真实接口样本
2. 选一个协议族打通一条真实上传成功样本
3. 把真实联调记录按 `docs/06-REAL_PROVIDER_SMOKE_TEMPLATE.md` 持续沉淀
