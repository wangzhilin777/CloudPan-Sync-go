...
| Runtime Evidence | 最近结果、最近探针、状态快照、状态矩阵 API、协议族覆盖矩阵、验收报告生成与历史保存、真实 provider smoke 记录与协议组聚合矩阵、真实样本矩阵、accepted/pending 验收判定、acceptanceMissing 缺失原因、acceptanceAdvice 补齐建议、accepted/in_progress/pending 统计、`UploadSuccessGroups` 总览统计、smoke Markdown 导出与分类、验收矩阵状态筛选与样本/任务直达、源端删除记录聚合、后台补传全局等待态计数聚合 | 已完成 | 无 | 真实联调样本沉淀 |
