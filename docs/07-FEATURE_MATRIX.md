...
| Runtime Evidence | 最近结果、最近探针、状态快照、状态矩阵 API、协议族覆盖矩阵、验收报告生成与历史保存、真实 provider smoke 记录与协议组聚合矩阵、真实样本矩阵、accepted/pending 验收判定、acceptanceMissing 缺失原因、acceptanceAdvice 补齐建议、accepted/in_progress/pending 统计、`AcceptedSmokeGroups / InProgressSmokeGroups / PendingSmokeGroups / UploadSuccessGroups / UploadSuccessSamples` 总览统计、smoke Markdown 导出与分类、验收矩阵状态筛选与样本/任务直达、源端删除记录聚合、后台补传全局等待态计数聚合，以及 `autoRecoverWaitingAuthRefreshTasks / autoRecoverWaitingLocalRestoreTasks / autoRecoverWaitingManualTasks / autoRecoverWaitingRetryLimitTasks` 细粒度等待态；状态汇总直接带出 `executionMode / scanMode / sourceDeletePolicy`，状态矩阵也直接展示 `Scan Mode / Source Delete`，最近 Probe 表也直接展示 `Source Delete` | 已完成 | 无 | 真实联调样本沉淀 |
...
| 单元测试 | auth、planner、task、provider、workflow、web、UI smoke；provider 目录注册 / 协议族分组 / 缺凭证校验门禁 | 已完成 | 持续补样本 | 真实 provider 契约覆盖继续加深 |
...
| 真实联调验收 | 模板、流程、文档已具备，已补 accepted/pending 验收判定层与矩阵视图（真实上传成功样本 + 该协议组已有任务覆盖），并能单独标记协议组是否已具备真实上传成功样本、以及已沉淀的上传成功样本数量 | 部分完成 | 首批真实样本沉淀中 | 每个协议族至少一条真实成功样本 |
