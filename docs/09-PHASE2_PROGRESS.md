...
> 2026-06-08 更新：下面按 [08-PHASE2_TODO.md](/E:/Workspace/VSCode/CloudPan-Sync-go/docs/08-PHASE2_TODO.md:1) 的 12 个里程碑拆分，不再只用一个容易误导的百分比。

| 主线 | 里程碑 | 当前状态 | 说明 |
| --- | --- | --- | --- |
| 主线一 | 每个协议族至少 1 条真实上传成功样本 | 进行中 | 缺口已经能通过 `smokeMatrixMissingUploadGroups` 直接定位，但真实样本本体仍需要真实账号和上传链路补齐。 |
| 主线一 | 每家 provider 最小异常样本 | 进行中 | provider 级缺基础、缺上传、缺异常、缺代表性样本已结构化汇总，但异常样本本体还没有全部补齐。 |
| 主线一 | 大文件 / 多层目录 / 重试恢复代表性样本 | 进行中 | 代表性缺口已经可在协议组和 provider 两层直接查看，仍缺完整真实样本完成清单。 |
| 主线一 | 固定记录模板便于回归复用 | 已完成 | 模板、结构化字段、复用优先级、回归入口、筛选和 preferred sample 摘要均已接入。 |
| 主线二 | 后台补传候选池调度策略 | 基本完成 | 已覆盖 mode / lane / protocol group / provider / profile 预算和轮转，仍需要更多真实长跑样本证明长期公平性。 |
| 主线二 | upload checkpoint / 续传恢复链路 | 基本完成 | checkpoint 数量、续传上下文、分片进度、readiness 和首要动作已进入运行证据、报告与页面。 |
| 主线二 | 大文件失败后默认稳定恢复 | 进行中 | 机制和回归测试已增强，但“默认稳”还需要真实大文件失败/续传成功样本闭环。 |
| 主线二 | 暂停 / 恢复 / 重试 / 子树级补传协同 | 基本完成 | selected pending / retry / directory subset 与 checkpoint 保留已有回归证明，后续主要靠真实场景继续压测。 |
| 主线三 | 回收各 provider 默认节流建议 | 进行中 | provider 默认风控校准 summary 已形成，但仍缺真实样本反推后的最终默认表。 |
| 主线三 | 按协议族整理推荐参数 | 基本完成 | request interval、directory interval、retry limit、risk keywords、auto retry window 来源和缺口已可核对。 |
| 主线三 | 高风险 provider 账号级预算建议 | 进行中 | 账号默认来源与建议已经展示，尚未形成所有高风险 provider 的完整验收记录。 |
| 主线三 | 样本结论同步回推荐语义和默认模板 | 已完成 | 控制台推荐语义、默认模板校准摘要、报告页和 workflow JSON 已接入。 |

### 更准确的进度判断

- 如果按“本地工程支撑能力、接口字段、报告和页面可观测性”看，二期已经进入收尾段，大约是 `85%+`。
- 如果按原始验收标准里的“真实 provider 样本全部补齐”看，二期仍是 `65% - 70%`，主要卡在真实账号、真实上传成功、异常与大文件恢复样本。
- 后续不建议继续堆同义摘要字段；除非拿到真实样本数据，否则本地代码能推进的空间已经明显变小。
...
- 已把 smoke 记录样本类型补强到覆盖降级 / pending_manual 异常样本，并同步细化自动补传关注点，避免这类最关键的最小异常样本继续被泛化进普通异常标签。
...
- 当前轮次已把 provider 风控校准 summary 接入 RuntimeEvidence，并补 task 测试兜底，用于让运行证据和保存报告使用同一套校准聚合字段。
- 当前轮次已新增 smokeMatrixMissing*Groups 与 smokeMatrixPriorityActionCounts 的 workflow JSON 兜底，用于把协议组真实样本缺口从运行证据再推进到可回放接口结构。
