...
func TestServiceRecoverBlockedTasksAutoResumesUploadCheckpointQueue(t *testing.T) {
	...
	if !strings.Contains(report.Markdown, "自动补传恢复完成度") {
		t.Fatalf("expected auto recover readiness in report markdown, got %s", report.Markdown)
	}
...
func TestServiceProviderSmokeRecords(t *testing.T) {
	...
	if len(evidence.ProviderSmokeProviderMissingBasicProviders) != len(registry.List())-1 ||
		len(evidence.ProviderSmokeProviderMissingUploadProviders) != len(registry.List()) ||
		len(evidence.ProviderSmokeProviderMissingAnomalyProviders) != len(registry.List()) ||
		len(evidence.ProviderSmokeProviderMissingRepresentativeProviders) != len(registry.List()) {
		t.Fatalf("expected runtime provider smoke provider missing provider lists, got %#v", evidence)
	}
	if evidence.ProviderSmokeProviderPriorityActionCounts["补 1 条 provider 上传成功样本"] != 1 ||
		evidence.ProviderSmokeProviderPriorityActionCounts["补 1 条 provider 基础成功样本"] != len(registry.List())-1 {
		t.Fatalf("expected runtime provider smoke provider priority action counts, got %#v", evidence.ProviderSmokeProviderPriorityActionCounts)
	}
	...
	if len(report.Summary.ProviderSmokeProviderMissingBasicProviders) != len(registry.List())-1 ||
		len(report.Summary.ProviderSmokeProviderMissingUploadProviders) != len(registry.List()) ||
		len(report.Summary.ProviderSmokeProviderMissingAnomalyProviders) != len(registry.List()) ||
		len(report.Summary.ProviderSmokeProviderMissingRepresentativeProviders) != len(registry.List()) {
		t.Fatalf("expected report provider smoke provider missing provider lists, got %#v", report.Summary)
	}
	if report.Summary.ProviderSmokeProviderPriorityActionCounts["补 1 条 provider 上传成功样本"] != 1 ||
		report.Summary.ProviderSmokeProviderPriorityActionCounts["补 1 条 provider 基础成功样本"] != len(registry.List())-1 {
		t.Fatalf("expected report provider smoke provider priority action counts, got %#v", report.Summary.ProviderSmokeProviderPriorityActionCounts)
	}
	...
	if !strings.Contains(report.Markdown, "Provider 验收缺基础成功 provider") || !strings.Contains(report.Markdown, "Provider 验收缺上传成功 provider") || !strings.Contains(report.Markdown, "Provider 验收缺异常样本 provider") || !strings.Contains(report.Markdown, "Provider 验收缺代表性样本 provider") {
		t.Fatalf("expected provider smoke provider missing provider lists in report markdown, got %s", report.Markdown)
	}
	if !strings.Contains(report.Markdown, "Provider 验收首要动作分布") || !strings.Contains(report.Markdown, "provider 基础成功样本") {
		t.Fatalf("expected provider smoke provider priority action counts in report markdown, got %s", report.Markdown)
	}
	var found123 bool
...
func TestServiceProviderSmokeMatrixTracksUploadSuccessSample(t *testing.T) {
	...
	if !strings.Contains(report.Markdown, "| ProtocolGroup | Readiness | Upload Success | Task Coverage | Anomaly Coverage | Representative Coverage |") {
		t.Fatalf("expected smoke readiness summary table in report markdown, got %s", report.Markdown)
	}
...
func TestProviderSmokeSummaryPrefersHigherReusePrioritySample(t *testing.T) {
	...
	if summary[0].PreferredUploadSampleID != uploadRecord.ID {
		t.Fatalf("expected preferred upload sample id %s, got %s", uploadRecord.ID, summary[0].PreferredUploadSampleID)
	}
	if summary[0].PreferredUploadSampleTitle != "upload sample" {
		t.Fatalf("expected preferred upload sample title upload sample, got %s", summary[0].PreferredUploadSampleTitle)
	}
	if summary[0].PreferredUploadPriority != "直接回归" {
		t.Fatalf("expected preferred upload sample priority 直接回归, got %s", summary[0].PreferredUploadPriority)
	}
