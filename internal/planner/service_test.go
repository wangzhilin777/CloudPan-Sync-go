...
func TestProviderDefaultRiskTemplateIncludesAutoRetryWindowSummary(t *testing.T) {
	...
	if len(template.CalibrationCoveredFields) != 7 || len(template.CalibrationTargetFields) != 8 {
		t.Fatalf("expected calibration covered/target fields 7/8, got %+v", template)
	}
	...
	if !strings.Contains(template.CalibrationSampleAdvice, "auto retry window") {
		t.Fatalf("expected calibration sample advice to mention auto retry window, got %+v", template)
	}
}
