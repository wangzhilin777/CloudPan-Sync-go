...
func DefaultCatalog() []Adapter {
	...
	return []Adapter{
		...
		StaticAdapter{MetaInfo: Provider{Key: "baidu_netdisk", DisplayName: "Baidu Netdisk", ProtocolGroup: "baidu_netdisk", AuthModes: []string{"official_oauth", "manual_cookie"}, FastUploadInputs: []string{"md5", "size"}, FallbackModes: []string{"download_upload"}, ConflictPolicies: []ConflictPolicy{ConflictPolicyOverwriteExisting, ConflictPolicyAutoRenameNew}, SupportsOverwrite: false, SupportsAutoRename: true, OverwriteBehavior: "downgrade_to_auto_rename", Status: "planned"}, CapabilityInfo: fullCapability},
