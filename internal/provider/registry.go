...
func DefaultCatalog() []Adapter {
	...
	return []Adapter{
		NewGuangyaFamilyAdapter(Provider{Key: "guangya", DisplayName: "Guangya", ProtocolGroup: "guangya", AuthModes: []string{"web_login_capture", "manual_token"}, FastUploadInputs: []string{"md5", "size", "name"}, FallbackModes: []string{"download_upload"}, ConflictPolicies: []ConflictPolicy{ConflictPolicyOverwriteExisting, ConflictPolicyAutoRenameNew}, SupportsOverwrite: false, SupportsAutoRename: true, OverwriteBehavior: "downgrade_to_auto_rename", Status: "planned"}, fullCapability),
		NewOpenFamilyAdapter(Provider{Key: "aliyundrive_open", DisplayName: "Aliyun Drive Open", ProtocolGroup: "aliyun_123_open", AuthModes: []string{"official_oauth"}, FastUploadInputs: []string{"sha1", "size"}, FallbackModes: []string{"download_upload"}, ConflictPolicies: []ConflictPolicy{ConflictPolicyOverwriteExisting, ConflictPolicyAutoRenameNew}, SupportsOverwrite: true, SupportsAutoRename: true, OverwriteBehavior: "provider_managed", Status: "planned"}, fullCapability, true),
