...
func TestRoutesServeAppJSIncludesRetryEvidenceLabels(t *testing.T) {
	...
	if !strings.Contains(body, "retryUploadCheckpointCount") {
		t.Fatalf("expected retryUploadCheckpointCount evidence in app.js, got %q", body)
	}
	...
	if !strings.Contains(body, "已打开目标目录：") {
		t.Fatalf("expected target directory open success flash in app.js, got %q", body)
	}
	if !strings.Contains(body, "已跳转到目标目录：") {
		t.Fatalf("expected target directory breadcrumb success flash in app.js, got %q", body)
	}
	...
	if !strings.Contains(body, "provider-card-advanced") {
		t.Fatalf("expected provider advanced details container in app.js, got %q", body)
	}
	if !strings.Contains(body, "展开高级信息") {
		t.Fatalf("expected provider advanced summary text in app.js, got %q", body)
	}
