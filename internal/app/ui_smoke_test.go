...
func TestConsoleUISmokeMainline(t *testing.T) {
	...
	runStep(t, runCtx, "status evidence",
		...
		waitForText(`#auto-recover-last-result-summary`, "outcomes 预演可放行 1"),
		waitForText(`#auto-recover-last-result-summary`, "classes 普通重试失败 1"),
	...
	runStep(t, runCtx, "status decision actions",
		...
		waitForValue(`#auto-recover-limit-per-mode`, "1"),
		waitForValue(`#auto-recover-limit-per-lane`, "1"),
		waitForText(`#auto-recover-budget-summary`, "group 2"),
		waitForText(`#auto-recover-budget-summary`, "mode 1"),
		waitForText(`#auto-recover-budget-summary`, "lane 1"),
		waitForText(`#flash`, "已按决策采用建议预算：mode 1 / lane 1 / group 2 / provider 3 / profile 2"),
	...
	runStep(t, runCtx, "status snapshot panels",
		...
		waitForText("body", "RETRY SCOPE"),
		waitForText("body", "RETRY PATHS"),
		waitForText("body", "CALIBRATED"),
		waitForText("body", "OVERRIDE FIELDS"),
		waitForText("body", "retrySelectedPaths"),
